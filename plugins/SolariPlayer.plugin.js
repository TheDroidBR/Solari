...
class SolariPlayer {
    ...
    t(key) {
        const sysLang = document.documentElement.getAttribute('lang') || 'en';
        const lang = sysLang.startsWith('pt') ? 'pt-BR' : sysLang.startsWith('es') ? 'es' : 'en';
        return SolariPlayer.translations[lang]?.[key] || SolariPlayer.translations['en'][key] || key;
    }
    ...
    checkForUpdates() {
        ...
        fetch(`${meta.updateUrl}?t=${Date.now()}`)
            .then(res => {
            ...
            .then(code => {
                ...
                if (this.isNewerVersion(meta.version, remoteVersion)) {
                    this.showUpdateModal(remoteVersion, code);
                }
            })
            .catch(err => {
                console.error(`[${meta.name}] Update check failed:`, err);
            });
    }
    ...
    showUpdateModal(remoteVersion, code) {
        ...
        const content = React.createElement("div", {
            ...
            React.createElement("p", { style: { marginBottom: "12px" } },
                this.t('updateDesc').replace('{name}', name)
            ),
            React.createElement("div", {
                ...
                React.createElement("div", {},
                    ...
                    React.createElement("div", { style: { fontSize: "16px", fontWeight: "bold", color: "#ef4444" } }, `v${version}`)
                ),
                React.createElement("div", {},
            ...
            content,
            {
                ...
                onConfirm: () => {
                    ...
                    fs.writeFile(pluginPath, code, "utf8", (err) => {
                        if (err) {
                            console.error(`[${name}] Failed to write update:`, err);
    ...
    checkChangelog() {
        try {
            const name = this.getName();
            const version = this.getVersion();
            const lastVersion = BdApi.Data.load(SolariPlayer.PLUGIN_ID, 'lastVersion');
            if (lastVersion && this.isNewerVersion(lastVersion, version)) {
                ...
                fetch(`${metaUrl}?t=${Date.now()}`)
                            ...
                            if (idx !== -1) {
                                ...
                                if (lines.length > 0) {
                                    BdApi.UI.showChangelogModal({
                                        title: name,
                                        subtitle: `v${version}`,
                                        blurb: this.t('updateSuccess').replace('{version}', version),
                                        changes: [
                                            {
                    ...
                    .catch(err => console.error(`[${name}] Failed to show changelog:`, err));
            }
            BdApi.Data.save(SolariPlayer.PLUGIN_ID, 'lastVersion', version);
        } catch (e) {
            console.error(`[${this.getName()}] Error in checkChangelog:`, e);
        }
    }
