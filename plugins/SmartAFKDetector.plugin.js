...
module.exports = class SmartAFKDetector {
    ...
    static translations = {
        ...
        es: {
            ...
            active: 'Activo',
            afkLevel: 'Nivel AFK',
            afkLevels: 'Niveles AFK',
            afkLevelsHint: 'Configura múltiples niveles AFK con diferentes tiempos y estado:',
            level: 'Nivel',
            minutes: 'min →',
            statusPlaceholder: 'Texto del estado',
            addLevel: '+ Añadir Nivel',
            done: '💾 Listo',
            savedSynced: '✓ ¡Guardado y sincronizado!',
            language: 'Idioma',
            returnedFromAFK: 'Volvió del AFK',
            statusChangedTo: 'Estado cambiado a:',
            customStatusChangedTo: 'Estado personalizado cambiado a:',
            cleared: '(limpio)',
            systemActivityDetected: 'Actividad del sistema detectada (via Solari)',
            levelUp: 'Subió al Nivel',
            welcomeBack: 'SmartAFK: ¡Bienvenido de vuelta!',
            errorModuleNotFound: 'Error: Módulo de estado no encontrado.',
            connectedToSolari: 'Conectado a Solari',
            connectedToSolariToast: 'SmartAFK: ¡Conectado a Solari!',
            disconnectedFromSolari: 'Desconectado de Solari',
            configUpdatedByApp: 'Configuraciones actualizadas por la App',
            configUpdatedToast: 'SmartAFK: ¡Config Actualizada!',
                ...
                onConfirm: () => {
                    ...
                    fs.writeFile(pluginPath, code, "utf8", (err) => {
                        if (err) {
                            ...
                            this.safeShowToast(`❌ Error: ${err.message}`, { type: "error" });
                            return;
                        }
                        this.safeShowToast(`✨ ${this.t('updateSuccess').replace('{version}', remoteVersion)}`, { type: "success" });
                    });
                }
            }
        );
    ...
    checkChangelog() {
        try {
            const lastVersion = BdApi.Data.load('SmartAFKDetector', 'lastVersion');
            if (lastVersion && this.isNewerVersion(lastVersion, this.meta.version)) {
                ...
                fetch(`${metaUrl}?t=${Date.now()}`)
                    ...
                    .catch(err => console.error('[SmartAFK] Failed to show changelog:', err));
            }
            BdApi.Data.save('SmartAFKDetector', 'lastVersion', this.meta.version);
        } catch (e) {
            console.error('[SmartAFK] Error in checkChangelog:', e);
        }
    }
