...
function installUpdateAndRestart(updatePath) {
    ...
    const batContent = [
        ...
        '',
        ':: Step 3: Check Update Type (Portable vs Installer)',
        'echo "%~nx2" | find /I "Setup" > nul',
        'if errorlevel 1 (',
        '    :: --- PORTABLE UPDATE PATH ---',
        `    echo [UPDATE] Portable update detected. Copying new version... >> "${logPath}"`,
        ...
        `    echo [UPDATE] Starting new Solari Portable... >> "${logPath}"`,
        `    start "" "${currentExe}"`,
        ') else (',
        '    :: --- INSTALLER MIGRATION PATH ---',
        `    echo [UPDATE] Installer Setup detected (Migration 1.7.0+). Executing silent install... >> "${logPath}"`,
        `    start /wait "" "${updatePath}" /S`,
        `    echo [UPDATE] Silent installation complete. >> "${logPath}"`,
        ')',
        '',
        ':: Step 5: Cleanup',
